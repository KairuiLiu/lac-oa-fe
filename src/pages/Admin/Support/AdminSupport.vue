...
const findList = async (condition = null) => {
	...
	const res = (await adminApi.reqGoodList({ token: '123', pageId: tablePage.currentPage, pageSize: tablePage.pageSize, condition, supportId: state.supportId })) as IAjaxRestlt;
	if (res.code) {
...
async function handleLog(goodId) {
	const res = (await adminApi.reqGoodLog({ token: '213', supportId: state.supportId, goodId })) as IAjaxRestlt;
	if (res.code) {
		message.error('数据请求失败');
	}
